...
export default function CommunityFeed() {
  ...
  const tagFilters = useMemo(() => {
    ...
    questions.forEach((q) => q.tags.forEach((tag) => uniqueTags.add(tag)));
  ...
  const filtered = useMemo(() => {
    return questions
      .map((q) => ({
        ...q,
        answersCount: answers.filter((a) => a.questionId === q.id).length,
      }))
      .filter((question) => {
        const matchesTab =
          tab === "recent"
      ...
      .sort((a, b) => {
        ...
        if (tab === "recent") return 0; // already pseudo-recent
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4 shadow-sm">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Filter size={16} />
          <span>Select a tab to explore questions from the community.</span>
        </div>
        ...
        <Tabs.Content value={tab} className="mt-4 space-y-4">
          ...
          {filtered.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border bg-card p-6 text-center text-muted-foreground">
              No questions yet. Try another filter.
            </div>
          ) : (
            <div className="space-y-4">
              {filtered.map((question) => {
                const answersCount = answers.filter((a) => a.questionId === question.id).length;
                const excerpt =
                        ...
                        <div className="space-y-3">
                          ...
                          <form
                            ...
                            onSubmit={(event) => {
                              ...
                              if (!draft) return;
                              addAnswer(question.id, { author: currentUser.name, body: draft });
